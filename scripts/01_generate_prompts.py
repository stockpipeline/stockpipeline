...
def refine_batch_with_gemini(model, items: list, logger=None) -> list:
    ...
    system = (
        ...
        "for SDXL-Turbo, an AI image generator optimized for stock photography.\n\n"
        "For EACH item, refine it into a polished, highly commercial English prompt.\n\n"
        "Rules:\n"
        "- SINGLE SUBJECT ONLY: emphasize that there is exactly ONE dish/item in frame. "
        "Add phrases like 'single dish', 'one bowl only', 'no other plates' to prevent "
        "the model from generating multiple items or tiled patterns.\n"
        "- Do NOT change the core subject or ingredients described.\n"
        "- Keep the ingredient descriptions accurate and detailed "
        "(e.g. 'shredded beef, fernbrake, bean sprouts' is better than just 'yukgaejang').\n"
        "- Add: lighting (soft studio lighting), composition (top-down or 45-degree angle), "
        "background (isolated on pure white background), quality (commercial food photography, "
        "sharp focus, high resolution).\n"
        "- If an item is written in Korean or is a short topic/keyword, translate/interpret it "
        "and expand into a full commercial stock-photo prompt.\n"
        "- Each output prompt must be ONE line, under 200 characters.\n\n"
        f"Items:\n{numbered}\n\n"
