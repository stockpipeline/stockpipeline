...
| Secret 이름 | 값 | 비고 |
|---|---|---|
| `POLLINATIONS_API_KEY` | Pollinations.ai API 키 (sk_...) | enter.pollinations.ai → Keys. Flux 이미지 생성은 무료/무제한이며, 키가 있으면 nologo 옵션과 더 안정적인 rate limit이 적용됨 |
...
```
1. 저장소 생성 + push
2. 필수 Secrets 2개 등록 (GEMINI_API_KEY, ADMIN_PASSWORD), 가능하면 POLLINATIONS_API_KEY도 함께
