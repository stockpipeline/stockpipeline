...
| Secret 이름 | 값 | 발급 위치 |
|---|---|---|
| `GEMINI_API_KEY` | Gemini API 키 (이미지 생성 Imagen 4 + 메타데이터/Vision 체크에 모두 사용) | aistudio.google.com/apikey |
| `ADMIN_PASSWORD` | 대시보드 접속 비밀번호 (직접 정하기) | - |

### 이미지 생성 (Imagen 4, 무료 티어)

이미지 생성은 Gemini API의 Imagen 4 모델을 사용한다 (GEMINI_API_KEY 재사용, 별도 키 불필요).

```
imagen-4.0-generate-001       무료 RPD 25
imagen-4.0-fast-generate-001  무료 RPD 25
imagen-4.0-ultra-generate-001 무료 RPD 25
─────────────────────────────────────
합계 75/일 (Generate → Fast → Ultra 순으로 폴백)
```

실제 한도는 aistudio.google.com/rate-limit 에서 확인 가능.
...
```
1. 저장소 생성 + push
2. 필수 Secrets 2개 등록 (GEMINI_API_KEY, ADMIN_PASSWORD) - Imagen 4 이미지 생성도 GEMINI_API_KEY로 동작
